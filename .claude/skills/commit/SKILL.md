...
2. **Run pre-commit checks**: Before committing, run the project's quality checks:
   ```bash
...
4. **Check for unrelated changes**: Analyze the diff for logically distinct units of work. Changes are unrelated if they have **different purposes** (e.g. a bug fix and a new feature, a refactor and a test addition, two independent features). Signs of unrelated changes:
   ...
   - Mix of conventional commit types (e.g. `fix:` + `feat:` + `test:` + `refactor:`)
...
5. **Detect issue reference**:
   - Check `$ARGUMENTS` for `--issue <number>` or `#<number>`
   - If not in arguments, check `.claude/plans/*.yaml` for any plan with `done: false` tasks that has an `issue` field
   - If an issue number is found, it will be appended to the commit message body

6. **Draft the commit message** following these rules:
   - Use conventional commits format: `type(scope): description`
   - Valid types: `feat`, `fix`, `refactor`, `test`, `docs`, `chore`, `ci`, `perf`, `style`
   - Scope (optional): feature name, component, or subsystem (e.g., `i18n`, `navbar`, `contact-form`)
   - Description: imperative mood ("add validation" not "added validation"), lowercase, no period at end
   - First line under 72 characters
   - Separate subject from body with a blank line if body is needed
   - Focus on WHY the change was made, not just WHAT changed
   - If an issue number was detected, append `Closes #<number>` in the commit body (not the subject line):
     ```
...
7. **Create the commit**: Use a HEREDOC to pass the message:
   ```bash
...
8. **Report**: Show the commit hash, message, and files changed. If multiple commits were created, show all of them.
